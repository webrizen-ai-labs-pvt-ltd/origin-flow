...
import { X, Plus, Trash2, CheckCircle2, Sparkles, Shield, Users, HardDrive } from "lucide-react";
import { Button } from "@origin-flow/ui";
import { PlanService, type BillingCycle, type CreatePlanInput } from "../../services/plan-service";
...
export function CreatePlanModal({ isOpen, onClose, onCreated }: CreatePlanModalProps) {
  ...
  const [features, setFeatures] = useState<string[]>([
    "Complete GST & PAN compliance workflows",
    "Multi-member role & hierarchy management",
    "Real-time passkey & biometric authentication",
    "Audit compliance & ledger reports",
  ]);
  ...
  const handleNameChange = (name: string) => {
    const slug = name
      ...
      .replace(/[^a-z0-9\s-]/g, "")
      .replace(/\s+/g, "-");
    setFormData((prev) => ({ ...prev, name, slug: prev.slug === "" || prev.slug === prev.name.toLowerCase().replace(/\s+/g, "-") ? slug : prev.slug }));
  };
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const payload: CreatePlanInput = {
        ...
        price: Math.round(Number(formData.priceInRupees) * 100), // Convert ₹ to Paise
        currency: "INR",
      ...
      setErrorMsg(err.response?.data?.error || "Failed to create subscription plan");
    } finally {
      ...
      <div className="relative w-full max-w-2xl rounded-2xl border border-secondary bg-primary shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between border-b border-secondary px-6 py-4 bg-secondary/20">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-semibold text-primary">Create Subscription Plan</h2>
              <p className="text-xs text-tertiary">Define tier pricing, billing cycle, quotas and feature matrix</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-primary">Slug (Identifier) *</label>
              <input
                type="text"
                required
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              placeholder="e.g. Tailored for growing chartered accounting firms & compliance teams"
          ...
          <div className="flex items-center gap-3 rounded-xl border border-secondary bg-secondary/10 p-3.5">
            ...
            />
            <label htmlFor="isPopular" className="text-xs font-medium text-primary cursor-pointer select-none">
              Mark as <strong>"Most Popular"</strong> badge on pricing cards
            </label>
          </div>
          ...
          <div className="space-y-3">
            <label className="text-xs font-semibold text-primary">Included Features Checklist</label>
            
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Type a feature item (e.g. 24/7 Priority Support)..."
        ...
        <div className="flex items-center justify-end gap-3 border-t border-secondary px-6 py-4 bg-secondary/10">
          ...
          <Button color="primary" onPress={() => handleSubmit()} isDisabled={isSubmitting || !formData.name || !formData.slug}>
